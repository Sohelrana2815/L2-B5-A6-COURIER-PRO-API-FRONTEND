...
export default function PublicParcelTracker({
  trackingId,
}: PublicParcelTrackerProps) {
  const [copied, setCopied] = useState(false);
  const { data: parcel, isLoading, error } = useTrackParcelQuery(trackingId);
    ...
    return (
      <div className="max-w-4xl mx-auto p-6">
        <Alert variant="destructive">
          ...
          <AlertDescription>
            Unable to find parcel with tracking ID:{" "}
    ...
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      <Card className=" border-muted-foreground">
        ...
        <CardContent>
          <div className="flex items-center gap-3">
            <div className="flex-1 p-3   rounded-md border border-gray-200 font-mono text-sm text-chart-2">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription className="text-muted-foreground">
            Follow your parcel's journey from start to finish
          </CardDescription>
        </CardHeader>
        <CardContent>
            ...
            {parcel.statusLogs.map(
              (
                ...
                >
                  ...
                  {/* Status dot */}
                  <div
                    className={`flex-shrink-0 w-12 h-12 rounded-full border-2 flex items-center justify-center ${getStatusColor(
                      log.status
                    )
                      .replace("bg-", "border-")
                      .replace("100", "200")}`}
